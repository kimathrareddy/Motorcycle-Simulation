...
global c
c = 1.2; %coefficient of friction on tires in corners, 0.6
...
heat_loss = zeros(length(current),1); %in watts using P = R*I^2

%account for heat loss of controller
Q_controller = 1800; %in W, from the emDrive Manual (assume average)
heat_loss_kJ_controller = tnet*Q_controller/1000;

%account for heat loss of battery
i = 1;
...
average_battery_heat_loss = (sum(heat_loss)/length(heat_loss));
disp("Average heat generated from battery in a lap (W): "+ average_battery_heat_loss);
disp("Max heat generated from battery in a lap (W): "+ max(heat_loss));
disp("Total heat generated from battery in a lap (kJ): "+ heat_loss_kJ);

%account for heat loss of motor
i = 1;
average_voltage = 100.8;
motor_heat_loss = zeros(length(current),1);

while i<=length(current)
    motor_heat_loss(i) = (1-MotEff)*current(i)*average_voltage; %P_elec = I*V, P_elec*(1-efficency) = Q
    i = i+1;
end

average_motor_heat_loss = sum(motor_heat_loss)/length(motor_heat_loss);
motor_heat_loss_kJ = sum(motor_heat_loss)*tStep/1000; %total heat loss in kJ

disp("Average heat generated from motor in a lap (W): "+ average_motor_heat_loss);
disp("Max heat generated from motor in a lap (W): "+ (1-MotEff)*max(current)*average_voltage);
disp("Total heat generated from motor in a lap (kJ): "+ motor_heat_loss_kJ);

%energy from battery in a lap

energy_used_per_lap = (sum(energy)*tStep/1000)/MotEff + heat_loss_kJ + heat_loss_kJ_controller; %MotEff for motor efficiency
disp("Energy used (KJ) from battery in a lap: "+energy_used_per_lap);

%amount of heat produced 
disp("Total amount of heat generated in a lap (kJ): " + (motor_heat_loss_kJ + heat_loss_kJ+heat_loss_kJ_controller))
disp("Average total amount of heat generated in a lap (W): " + (average_motor_heat_loss + average_battery_heat_loss + Q_controller))

%energy percentage left after 1 lap
disp("Energy left (fraction) after 1 lap: "+(1-(energy_used_per_lap/Energy_left)));

disp("Average battery (mechanical + heat) power needed (W): "+ (((sum(energy)/length(energy)))+(sum(heat_loss)/length(heat_loss))+average_motor_heat_loss+Q_controller));
